...
test('clinical binary UX opens optional observations only after Sim and procedures keep free notes',async({page})=>{
 ...
 await expect(hypertension.locator('textarea')).toHaveCount(0);await hypertension.getByRole('radio',{name:'Sim'}).click();await expect(hypertension.locator('textarea')).toBeVisible();
 const procedures=page.locator('#procedures');const limpeza=page.locator('#q-aesthetics-limpeza_pele');await expect(procedures.getByText('Limpeza de pele',{exact:true})).toBeVisible();await expect(procedures.locator('textarea#detail-aesthetics-limpeza_pele')).toHaveCount(0);await limpeza.getByRole('radio',{name:'Sim'}).click();await expect(procedures.locator('textarea#detail-aesthetics-limpeza_pele')).toBeVisible();await expect(procedures.locator('input[type="date"]')).toHaveCount(0);
