...
begin
  ...
  if not exists (select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='list_operational_reactivation_v1' and not p.prosecdef and array_to_string(p.proconfig,',') like '%search_path=public, pg_temp%' and pg_get_functiondef(p.oid) like '%list_relationship_opportunities_v1%') then raise exception 'Relationship bridge must be SECURITY INVOKER, fixed search_path and reuse Relationship 3.8 RPC'; end if;
