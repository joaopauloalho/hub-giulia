-- Hub Giulia 3.3 — preserve the backend-only security model for procedure_returns.
-- Dashboard RPCs consume the existing tenant-scoped returns API instead of opening table SELECT.
...
begin
  ...
  return (
    ...
    active_returns as (
      select r.* from public.list_procedure_returns_v2() r
      where r.completed_at is null and r.dismissed_at is null and (r.appointment_id is null or r.appointment_status = 'cancelado')
    ),
