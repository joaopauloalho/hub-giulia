-- Add the children question before pregnancy in Women's Health.
-- The answer remains inside surgical_history, preserving the current JSON model and
-- historical frozen snapshots. Only future finalized versions use this enriched schema.
...
as $function$
  with base as (
    ...
    select
      s.ordinality,
      case
        when s.section ->> 'key' = 'womens_health' then
          jsonb_set(
            s.section,
            '{fields}',
            jsonb_build_array(
              jsonb_build_object(
                ...
                'label', 'Tem filhos?',
                'type', 'boolean_detail',
                'detail_key', 'quantidade_filhos'
              )
            ) || coalesce(s.section -> 'fields', '[]'::jsonb),
            false
          )
