...
begin
  ...
  with contributing as (
    select pi.id as procedure_item_id,pi.service_id,pi.name as service_name_snapshot,
  ...
  select jsonb_agg(jsonb_build_object(
    ...
    ) order by service_id::text,procedure_item_id::text),
  ...
  with instruction_rows as (
    select btrim(pr.instructions) as body,min(pi.service_id::text) as first_service_id
    from public.procedure_items pi
    join public.service_aftercare_protocols pr on pr.service_id=pi.service_id and pr.user_id=pi.user_id and pr.enabled
  ...
  select string_agg(body,E'\n\n' order by first_service_id,body) into v_instructions from instruction_rows;
  ...
  select p_user_id,v_plan_id,p_procedure_id,p_procedure_id,v_patient_id,
    ...
    ) order by pi.service_id::text,pi.id::text,st.sort_order,st.id::text),
    'pending'
  from public.procedure_items pi
  join public.service_aftercare_protocols pr on pr.service_id=pi.service_id and pr.user_id=pi.user_id and pr.enabled
