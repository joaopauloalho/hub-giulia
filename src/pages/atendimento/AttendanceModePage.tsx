...
import { ArrowLeft, CalendarDays, Camera, CheckCircle2, ClipboardList, FileSignature, HeartPulse, RefreshCw, RotateCcw, Syringe, UserRound, WalletCards } from 'lucide-react';
...
export function AttendanceModePage() {
  ...
  const load = useCallback(async () => {
    ...
    if (contextError) {
      console.error('[attendance:context]', contextError);
      setContext(null);
      setError('Não foi possível carregar o atendimento. Tente novamente.');
    } else {
      const row = ((data ?? []) as AttendanceContext[])[0] ?? null;
      setContext(row);
      if (!row) setError('Atendimento não encontrado ou sem acesso.');
    }
    setLoading(false);
  }, [appointmentId]);
    ...
    <div className="page attendance-page">
      <header className="attendance-header">
        ...
        <div className="attendance-title-block">
          <div className="attendance-eyebrow">Modo Atendimento</div>
          <h1 className="page-title">{context.patient_name}</h1>
          <p className="page-sub">{dateTime(context.scheduled_at)} · {context.service_name ?? 'Serviço não definido'} · {context.duration_minutes} min</p>
        </div>
        <span className={`badge ${context.appointment_status === 'realizado' ? 'badge--green' : context.appointment_status === 'cancelado' || context.appointment_status === 'nao_compareceu' ? 'badge--red' : 'badge--rose'}`}>
          {APPOINTMENT_LABEL[context.appointment_status] ?? context.appointment_status}
        </span>
      </header>

      <section className="attendance-status-card card" aria-labelledby="attendance-status-title">
        <div className="attendance-section-heading">
          <div><h2 id="attendance-status-title">Status do atendimento</h2><p className="page-sub">Leitura factual dos módulos atuais. Nenhuma etapa é marcada como obrigatória por esta tela.</p></div>
          <button className="btn btn--ghost btn--sm" onClick={() => void load()}><RefreshCw size={14} /> Atualizar</button>
        </div>
      ...
      <footer className="attendance-footer card">
        <CalendarDays size={18} />
        <div><strong>Conclusão sem estado paralelo</strong><p className="page-sub">O Hub 3.5 não cria um botão mágico de conclusão. O status exibido acima continua vindo da Agenda e o procedimento continua vindo do Registrar.</p></div>
      </footer>
    </div>
  );
}
