...
export function PacienteView({ patient, archived = false, sourceAppointmentId, initialTab, onClose, onUpdate, onArchive, onRestore }: Props) {
  ...
  const schedule = () => navigate(`/agenda?patient_id=${patient.id}`, {
    state: { patient: { id: patient.id, name: patient.name, phone: patient.phone }, from: returnTo ?? `/pacientes/${patient.id}` },
  });
  ...
  const register = () => {
    ...
    navigate(`/registrar?${query.toString()}`, {
      state: { patient, patientId: patient.id, appointmentId: sourceAppointmentId, from: returnTo ?? `/pacientes/${patient.id}` },
    });
  };
  const attendance = () => {
    if (sourceAppointmentId) navigate(`/atendimento/${sourceAppointmentId}`);
  };
  const whatsapp = () => {
    if (!patient.phone) return;
    const url = buildWhatsAppUrl(patient.phone, `Olá ${patient.name}!`);
  ...
  const archiveOrRestore = async () => {
    const ok = await confirm({
      ...
      message: archived
        ? `Restaurar ${patient.name} para a lista ativa?`
        : `Arquivar ${patient.name}? O histórico clínico e financeiro será preservado e poderá ser restaurado.`,
  ...
  return <div className="drawer-overlay patient-360-overlay" onClick={onClose}>
    <aside className="drawer patient-360-drawer" role="dialog" aria-modal="true" aria-labelledby="paciente-view-title" onClick={event => event.stopPropagation()}>
      <div className="drawer-header">
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="drawer-title" id="paciente-view-title">
            {patient.name}
            {archived && <span className="badge badge--amber" style={{ marginLeft: 8 }}>Arquivada</span>}
          </div>
          <div className="drawer-sub" style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 12px', marginTop: 2 }}>
            ...
            {crmSummary && <button type="button" className="badge badge--rose" style={{ border: 0, cursor: 'pointer' }} onClick={() => navigate('/crm')}>
              <TrendingUp size={11} style={{ marginRight: 4 }} />CRM · {CRM_STAGE_LABEL[crmSummary.stage]}
              {crmSummary.next_followup_on ? ` · follow-up ${crmSummary.next_followup_on.split('-').reverse().join('/')}` : ''}
            </button>}
          </div>
        </div>
      </div>
      ...
      <div className="patient-quick-actions" aria-label="Ações da paciente">
        {sourceAppointmentId && <button className="btn btn--primary btn--sm" onClick={attendance}><HeartPulse size={15} /> Atendimento</button>}
        <button className={`btn btn--sm ${sourceAppointmentId ? 'btn--secondary' : 'btn--primary'}`} onClick={register}><ClipboardPlus size={15} /> Registrar</button>
        ...
        <button className="btn btn--ghost btn--sm patient-quick-action--low" onClick={crmOpportunity}><TrendingUp size={15} /> Oportunidade</button>
        <button className="btn btn--ghost btn--sm patient-quick-action--low" onClick={() => setTab('notes')}><StickyNote size={15} /> Nota</button>
        <button className="btn btn--ghost btn--sm patient-quick-action--low" onClick={() => setTab('data')}><Pencil size={15} /> Editar</button>
        <button className="btn btn--ghost btn--sm patient-quick-action--low" onClick={() => void archiveOrRestore()}>
          {archived ? <RotateCcw size={15} /> : <Archive size={15} />} {archived ? 'Restaurar' : 'Arquivar'}
        </button>
      </div>

      <div className="patient-360-content-shell">
        <nav className="sub-tabs patient-360-tabs" aria-label="Seções da paciente">
          {TABS.map(([label, key]) => <button key={key} className={`sub-tab${tab === key ? ' sub-tab--active' : ''}`} aria-current={tab === key ? 'page' : undefined} onClick={() => setTab(key)}>{label}</button>)}
        </nav>
        <div className="drawer-body">
          {tab === 'overview'
            ? <div style={{ display: 'grid', gap: 12 }}>
                <PatientAcquisitionCard patient={patient} />
                <OverviewTab patientId={patient.id} onAgenda={schedule} onReturns={() => navigate(`/retornos?patient_id=${patient.id}`)} onHistory={() => setTab('procedures')} onFinance={() => setTab('finance')} onNotes={() => setTab('notes')} onAnamnesis={() => setTab('anamnesis')} onTimeline={() => setTab('timeline')} />
                {!archived && <PatientWaitlistCard patient={{ id: patient.id, name: patient.name, phone: patient.phone }} />}
