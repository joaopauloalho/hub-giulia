import { AlertTriangle, CalendarDays, ClipboardCheck, CreditCard, FileClock, History, StickyNote, WalletCards } from 'lucide-react';
...
export function OverviewTab(props: Props) {
  ...
  const expiring = packageData.packages
    .filter(pkg => pkg.effective_status === 'active' && pkg.valid_until)
    .map(pkg => ({ ...pkg, days: differenceInCalendarDays(new Date(`${pkg.valid_until}T12:00:00`), new Date()) }))
    .filter(pkg => pkg.days >= 0 && pkg.days <= 30)
  ...
  return <div style={{ display: 'grid', gap: 12 }}>
    {alerts.length > 0 && <div className="card" style={{ padding: 12 }}>
      ...
      <div style={{ display: 'grid', gap: 6 }}>{alerts.map(alert => alert.action ? <button key={alert.key} type="button" onClick={alert.action} style={{ border: '1px solid var(--border)', borderRadius: 9, background: 'var(--bg-2)', padding: '9px 10px', textAlign: 'left', cursor: 'pointer', color: 'var(--text)' }}>{alert.label}</button> : <div key={alert.key} style={{ border: '1px solid var(--border)', borderRadius: 9, background: 'var(--bg-2)', padding: '9px 10px', color: 'var(--text)' }}>{alert.label}</div>)}</div>
    </div>}
    ...
    <div className="card" style={{ padding: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 11 }}><WalletCards size={16} /><strong style={{ flex: 1, fontSize: 13 }}>Pacotes & Créditos</strong>{!entitlements.loading && <span className="page-sub">{activeEntitlements.length} benefício{activeEntitlements.length === 1 ? '' : 's'} ativo{activeEntitlements.length === 1 ? '' : 's'}</span>}</div>
      {entitlements.loading || packageData.loading ? <Skeleton lines={3} /> : entitlements.data.length === 0 ? <p className="page-sub">A paciente ainda não possui pacotes ou créditos.</p> : <div style={{ display: 'grid', gap: 10 }}>
        {packageData.packages.slice(0, 6).map(pkg => {
          const items = entitlements.data.filter(item => item.package_id === pkg.package_id);
          return <div key={pkg.package_id} style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 11, background: 'var(--bg-2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, marginBottom: 7 }}><div><strong style={{ fontSize: 13 }}>{pkg.package_title}</strong><div className="page-sub">Origem: {sourceLabel(pkg.source_type)}{pkg.valid_until ? ` · válido até ${dateOnly(pkg.valid_until)}` : ''}</div></div><span style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: pkg.effective_status === 'active' ? 'var(--green)' : 'var(--text-3)' }}>{pkg.effective_status === 'active' ? 'Ativo' : pkg.effective_status === 'completed' ? 'Concluído' : pkg.effective_status === 'expired' ? 'Expirado' : pkg.effective_status === 'draft' ? 'Pendente' : 'Anulado'}</span></div>
            <div style={{ display: 'grid', gap: 5 }}>{items.map(item => <div key={item.package_item_id} style={{ display: 'flex', justifyContent: 'space-between', gap: 10, fontSize: 12.5 }}><span>{item.service_name_snapshot}</span><strong>{item.available_balance.toLocaleString('pt-BR')} de {item.quantity_granted.toLocaleString('pt-BR')} disponível{item.available_balance === 1 ? '' : 'is'}</strong></div>)}</div>
          </div>;
        })}
