...
function ProcedureCard({ proc, contracts, treatmentSessions, onPhotos, onInjectables, onContract }: { proc: Procedure; contracts: Contract[]; treatmentSessions: TreatmentSessionRecord[]; onPhotos?: () => void; onInjectables?: () => void; onContract?: (procedureId: string) => void }) {
  ...
  return <div className="card" style={{ overflow: 'hidden', marginBottom: 8 }}>
    <button type="button" onClick={() => setOpen(value => !value)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
      <div style={{ width: 40, height: 40, borderRadius: 10, flexShrink: 0, background: isTreatmentOnly ? '#f0fdf4' : 'var(--bg-2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><ClipboardList size={18} style={{ color: isTreatmentOnly ? '#166534' : 'var(--primary)' }} /></div>
      ...
      <div style={{ textAlign: 'right', flexShrink: 0 }}>{isTreatmentOnly ? <><strong style={{ color: '#166534', fontSize: 13 }}>Já pago</strong><div className="page-sub">sessão do tratamento</div></> : <><strong style={{ color: 'var(--primary)' }}>{currency(finance.venda)}</strong><div className="page-sub">recebido {currency(finance.pago)}</div></>}</div>
    ...
    {open && <div style={{ padding: '12px 16px 16px', borderTop: '1px solid var(--border)', display: 'grid', gap: 14 }}>
      <section>
        ...
        <div style={{ display: 'grid', gap: 8, marginTop: 8 }}>
          {items.length > 0 ? items.map(item => { const session = treatmentByItem.get(item.id); return <div key={item.id} style={{ display: 'flex', gap: 10, justifyContent: 'space-between', alignItems: 'flex-start' }}><span style={{ fontSize: 13 }}>{item.qty > 1 ? `${item.qty}× ` : ''}{item.name}{session && <span style={{ display: 'block', color: '#166534', fontSize: 11, fontWeight: 700, marginTop: 2 }}>{sessionLabel(session)} · {session.package_title}</span>}</span>{session ? <span style={{ fontSize: 11, fontWeight: 800, color: '#166534', whiteSpace: 'nowrap' }}>JÁ PAGO</span> : <strong style={{ fontSize: 13 }}>{currency(item.final_price * item.qty)}</strong>}</div>; }) : <span className="page-sub">Snapshots detalhados não disponíveis neste registro legado.</span>}
        </div>
      </section>

      <section>
        <strong style={{ fontSize: 12, textTransform: 'uppercase', letterSpacing: '.04em' }}>Pagamento deste atendimento</strong>
        <div style={{ display: 'grid', gap: 7, marginTop: 8 }}>
          {payments.length > 0 ? payments.map(payment => <div key={payment.id} style={{ display: 'flex', gap: 8, justifyContent: 'space-between', alignItems: 'center' }}><div><span style={{ fontSize: 13 }}>{paymentLabel(payment)}</span><div className="page-sub">{payment.paid_at ? `Recebido em ${new Date(payment.paid_at).toLocaleDateString('pt-BR')}` : payment.scheduled_date ? `Previsto para ${new Date(`${payment.scheduled_date}T12:00:00`).toLocaleDateString('pt-BR')}` : 'Pendente'}</div></div><strong style={{ fontSize: 13 }}>{currency(payment.amount)}</strong></div>) : treatmentSessions.length > 0 ? <div style={{ padding: 10, borderRadius: 9, background: '#f0fdf4', border: '1px solid #bbf7d0', color: '#166534', fontSize: 12 }}><strong>Nenhuma nova cobrança.</strong> O pagamento foi registrado na venda do tratamento.</div> : <span className="page-sub">Nenhum pagamento detalhado disponível.</span>}
        </div>
      </section>

      {!isTreatmentOnly && <section style={{ display: 'grid', gap: 5 }}><div style={{ display: 'flex', justifyContent: 'space-between' }}><span className="page-sub">Valor vendido</span><strong>{currency(finance.venda)}</strong></div><div style={{ display: 'flex', justifyContent: 'space-between' }}><span className="page-sub">Recebido</span><strong>{currency(finance.pago)}</strong></div><div style={{ display: 'flex', justifyContent: 'space-between' }}><span className="page-sub">Pendente</span><strong>{currency(finance.pendente)}</strong></div></section>}
...
export function HistoricoTab({ patientId, onPhotos, onInjectables, onContract }: Props) {
  ...
  return <div style={{ padding: '0 0 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, padding: '10px 14px', background: 'var(--bg-2)', borderRadius: 10, border: '1px solid var(--border)' }}><span className="page-sub">{procedures.length} atendimento{procedures.length !== 1 ? 's' : ''}</span><strong style={{ color: 'var(--primary)' }}>Vendas nos atendimentos: {currency(total)}</strong></div>{procedures.map(proc => <ProcedureCard key={proc.id} proc={proc} contracts={contractsByProcedure.get(proc.id) ?? []} treatmentSessions={sessionsByProcedure.get(proc.id) ?? []} onPhotos={onPhotos} onInjectables={onInjectables} onContract={onContract}/>)}</div>;
}
