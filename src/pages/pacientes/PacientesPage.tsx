...
export function PacientesPage() {
  ...
  return <div className="page">
    ...
    <div style={{ display: 'flex', gap: 7, marginBottom: 10 }}>
      <button className={`btn btn--sm ${archiveMode === 'active' ? 'btn--primary' : 'btn--ghost'}`} onClick={() => { setArchiveMode('active'); setViewing(null); navigate('/pacientes', { replace: true }); }}>Ativas</button>
      <button className={`btn btn--sm ${archiveMode === 'archived' ? 'btn--primary' : 'btn--ghost'}`} onClick={() => { setArchiveMode('archived'); setViewing(null); navigate('/pacientes', { replace: true }); }}><Archive size={14} /> Arquivadas</button>
    </div>
