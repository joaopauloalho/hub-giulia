...
export function AgendaFormDrawer({ initialDate, initialTime, initialPatientId, initialPatient, appointment, findConflict, onCreate, onUpdate, onClose }: {
  initialDate: string;
  initialTime: string;
  ...
  return <div className="drawer-overlay" onClick={() => !saving && onClose()}><div className="drawer" role="dialog" aria-modal="true" aria-labelledby="agenda-form-title" onClick={event => event.stopPropagation()}>
    ...
    <div className="drawer-body">
      ...
      <label className="field-label">Serviço</label><select className="field-input" value={serviceId} onChange={event => selectService(event.target.value)}><option value="">Consulta / a definir</option>{servicos.filter(service => service.active).map(service => <option key={service.id} value={service.id}>{service.name}{service.duration_minutes ? ` · ${service.duration_minutes} min` : ''}</option>)}</select>
