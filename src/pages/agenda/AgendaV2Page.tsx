...
import { useAgenda, type AgendaAppointment, type AgendaInput, type AgendaStatus } from '../../hooks/useAgenda';
import { fulfillPatientWaitlist, type WaitlistCandidate } from '../../hooks/useAgendaRecovery';
...
import { WaitlistCandidatesDrawer, WaitlistEditor } from './AgendaRecoveryUI';
...
export function AgendaV2Page() {
  ...
  const [waitlistAppointment, setWaitlistAppointment] = useState<AgendaAppointment | null>(null);
  const [candidateSlot, setCandidateSlot] = useState<AgendaAppointment | null>(null);
  ...
  const fulfillAfterSave = async (saved: AgendaAppointment) => {
    if (!form?.waitlistEntryId) return;
    try { await fulfillPatientWaitlist(form.waitlistEntryId, saved.id); }
    catch (error) {
      console.error('[agenda-recovery] appointment saved but waitlist fulfillment failed', error);
      toast.error('Consulta salva. A lista de encaixe não foi concluída automaticamente; atualize a página para conferir.');
    }
  };

  const createFromForm = async (input: AgendaInput) => {
    const saved = await agenda.create(input);
    await fulfillAfterSave(saved);
    return saved;
  };

  const updateFromForm = async (id: string, input: Partial<AgendaInput>) => {
    const saved = await agenda.update(id, input);
    await fulfillAfterSave(saved);
    return saved;
  };

  const scheduleCandidate = (candidate: WaitlistCandidate, sourceAppointment: AgendaAppointment | null) => {
    if (!candidateSlot) return;
    const slotDate = clinicDateIso(candidateSlot.scheduled_at);
    const slotTime = clinicTime(candidateSlot.scheduled_at);
    if (sourceAppointment) {
      setForm({
        date: slotDate,
        time: slotTime,
        appointment: { ...sourceAppointment, scheduled_at: candidateSlot.scheduled_at },
      ...
      setForm({
        ...
        time: slotTime,
        appointment: null,
        initialPatientId: candidate.patient_id,
        initialPatient: { id: candidate.patient_id, name: candidate.patient_name, phone: candidate.phone },
        initialServiceId: candidate.service_id ?? candidateSlot.service_id,
        initialDuration: candidateSlot.duration_minutes ?? 60,
        waitlistEntryId: candidate.entry_id,
      });
    }
  ...
  return <div className="page">
    ...
    {form && <AgendaFormDrawer initialDate={form.date} initialTime={form.time} initialPatientId={form.initialPatientId} initialPatient={form.initialPatient} initialServiceId={form.initialServiceId} initialDuration={form.initialDuration} appointment={form.appointment} findConflict={agenda.findConflict} onCreate={createFromForm} onUpdate={updateFromForm} onClose={() => setForm(null)} />}
    {detail && <AppointmentDetailDrawer appointment={detail} isReturn={detail.source === 'return' || returnIds.has(detail.id)} googleConnected={google.connected} needsReauth={google.needsReauth} onEdit={() => { const item = detail; setDetail(null); setForm({ date: clinicDateIso(item.scheduled_at), time: clinicTime(item.scheduled_at), appointment: item }); }} onConfirm={() => agenda.confirm(detail.id)} onCancel={reason => agenda.cancel(detail.id, reason)} onNoShow={() => agenda.markNoShow(detail.id)} onRetryGoogle={() => agenda.retryGoogle(detail.id)} onWaitlist={() => setWaitlistAppointment(detail)} onCandidates={() => setCandidateSlot(detail)} onClose={() => { setDetail(null); void agenda.refresh(); }} />}
    {waitlistAppointment && <WaitlistEditor patient={{ id: waitlistAppointment.patient_id, name: waitlistAppointment.patient?.name ?? 'Paciente', phone: waitlistAppointment.patient?.phone }} sourceAppointment={waitlistAppointment} onClose={() => setWaitlistAppointment(null)} />}
    {candidateSlot && <WaitlistCandidatesDrawer slot={candidateSlot} onSchedule={scheduleCandidate} onClose={() => setCandidateSlot(null)} />}
