...
function DayTimeline({ rows, onOpen, onCreate }: { rows: AgendaAppointment[]; onOpen: (appointment: AgendaAppointment) => void; onCreate: (time: string) => void }) {
  ...
  return <div style={{ borderTop: '1px solid var(--border)' }}>{hours.map(hour => {
    ...
    return <div key={hour} style={{ display: 'grid', gridTemplateColumns: '58px 1fr', minHeight: 62, borderBottom: '1px solid var(--border)' }}><button type="button" onClick={() => onCreate(hour)} style={{ border: 0, background: 'transparent', color: 'var(--text-3)', cursor: 'pointer' }}>{hour}</button><div style={{ borderLeft: '1px solid var(--border)', padding: 6, display: 'grid', gap: 6 }}>{items.map(item => <Event key={item.id} appointment={item} onOpen={() => onOpen(item)} />)}{items.length === 0 && <button type="button" onClick={() => onCreate(hour)} style={{ minHeight: 40, border: 0, background: 'transparent', cursor: 'pointer' }} aria-label={`Agendar ${hour}`} />}</div></div>;
  })}</div>;
}
