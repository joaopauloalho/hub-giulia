      ...
      <div className="drawer" role="dialog" aria-modal="true" aria-labelledby="appointment-detail-title" onClick={event => event.stopPropagation()}>
        ...
        <div className="drawer-body">
          <div style={{ display: 'grid', gap: 10 }}>
            ...
            {appointment.status === 'cancelado' && <div className="card" style={{ padding: 12, border: '1px solid var(--border)' }}><div style={{ display: 'flex', gap: 8, alignItems: 'center' }}><BellRing size={17}/><strong>Horário disponível</strong></div><p className="page-sub" style={{ marginTop: 4 }}>Veja quem já disse que gostaria de antecipar um atendimento.</p><button type="button" className="btn btn--primary btn--sm" style={{ marginTop: 8, minHeight: 44 }} onClick={onCandidates}>Ver lista de encaixe</button></div>}
          </div>
        ...
        <div className="drawer-footer" style={{ flexWrap: 'wrap' }}>
          ...
          {active && <button className="btn-secondary" onClick={onWaitlist} disabled={busy}><BellRing size={15} /> Lista de encaixe</button>}
