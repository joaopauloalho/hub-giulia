...
export function TreatmentExecutionStep({
  catalogServices,
  loadingServices,
  selected,
  entitlements,
  ...
  const filteredExtras = useMemo(() => {
    ...
    return activeServices.filter(service => !normalized || service.name.toLocaleLowerCase('pt-BR').includes(normalized));
  ...
  return <div className="treatment-execution">
    <div className="treatment-execution__heading">
      <div>
        <span className="treatment-execution__eyebrow"><Sparkles size={14}/> Atendimento de hoje</span>
        <h2>O que foi realizado hoje?</h2>
        <p>Tratamentos já pagos aparecem primeiro. Selecione somente as sessões feitas hoje; o Hub controla a sequência e não cobra novamente.</p>
      </div>
      {(coveredCount > 0 || extraCount > 0) && <div className="treatment-execution__selection-summary" aria-live="polite">
        {coveredCount > 0 && <span>{coveredCount} do tratamento</span>}
        {extraCount > 0 && <span>{extraCount} extra{extraCount === 1 ? '' : 's'}</span>}
      </div>}
    </div>

    {loadingEntitlements ? <div className="treatment-execution__loading"><Loader2 size={20} className="spin"/> Verificando tratamentos ativos…</div> : hasPlans ? <div className="treatment-plan-list">
      {plans.map(plan => {
        ...
        return <section className="treatment-plan" key={plan.packageId}>
          <header className="treatment-plan__header">
            ...
            <div className="treatment-plan__title-wrap">
              <strong>{plan.title}</strong>
              <span>{plan.completedSessions.toLocaleString('pt-BR')} de {plan.totalSessions.toLocaleString('pt-BR')} sessões realizadas · {plan.remainingSessions.toLocaleString('pt-BR')} restantes</span>
            </div>
          ...
          <div className="treatment-plan__items">
            {plan.items.map(item => {
              ...
              return <button
                ...
                type="button"
                className={`treatment-session${selectedHere ? ' treatment-session--selected' : ''}`}
                onClick={() => service && selectable && onTreatmentToggle(service, item)}
                disabled={!selectable}
                aria-pressed={selectedHere}
              >
                <span className="treatment-session__check">{selectedHere ? <Check size={15} strokeWidth={3}/> : null}</span>
                <span className="treatment-session__main">
                  <strong>{item.service_name_snapshot}</strong>
                  <span>{treatmentProgressLabel(item)}</span>
                  {!service && <small>Serviço original não está mais disponível no catálogo.</small>}
                  {service && !service.active && <small>Serviço inativo no catálogo.</small>}
                </span>
                <span className="treatment-session__next">
                  <small>{selectedHere ? 'Selecionado para hoje' : 'Próxima'}</small>
                  <strong>{treatmentSessionLabel(item)}</strong>
                </span>
              </button>;
            })}
          </div>
    ...
    </div> : <div className="treatment-execution__empty">
      <strong>Nenhum tratamento com sessões disponíveis</strong>
      <span>Selecione abaixo o procedimento avulso realizado hoje.</span>
    </div>}

    {hasPlans && <button type="button" className="treatment-execution__extras-toggle" onClick={() => setExtrasOpen(value => !value)} aria-expanded={showExtras}>
      <span><Plus size={16}/> Adicionar procedimento fora do tratamento</span>
      {showExtras ? <ChevronDown size={17}/> : <ChevronRight size={17}/>} 
    </button>}

    {showExtras && <section className="treatment-execution__extras">
      <div className="treatment-execution__extras-copy">
        <strong>{hasPlans ? 'Procedimento extra' : 'Procedimentos'}</strong>
        <span>{hasPlans ? 'Itens selecionados aqui entram como cobrança adicional.' : 'Escolha o que foi realizado hoje.'}</span>
      </div>
      <div className="treatment-execution__search">
        <Search size={16}/>
        <input value={query} onChange={event => setQuery(event.target.value)} placeholder="Buscar procedimento…" aria-label="Buscar procedimento"/>
      </div>
      {loadingServices ? <div className="treatment-execution__loading"><Loader2 size={20} className="spin"/> Carregando catálogo…</div> : <div className="treatment-extra-list">
        {filteredExtras.map(service => {
          ...
          return <button
            key={service.id}
            type="button"
            className={`treatment-extra${selectedExtra ? ' treatment-extra--selected' : ''}`}
            onClick={() => !coveredElsewhere && onExtraToggle(service)}
