...
export function TreatmentRegistrarPageV2() {
  ...
  const saveDraftNow = async () => {
    if (!patient) return;
    const payload: AttendanceDraftPayload = { performedDate, notes, serviceIds: services.map(service => service.id), coverageByService: coverage, finalPriceByService: prices, costByService: costs, courtesyByService: courtesy, materials, paymentTiming: timing, payments };
  ...
  const confirmAttendance = async () => {
    ...
    try {
      ...
      const procedure = await create({
        ...
        total_cost: services.reduce((sum, service) => sum + Number(costs[service.id] ?? service.cost_per_unit ?? 0), 0),
