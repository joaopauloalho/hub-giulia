...
import { CalendarDays, Check, CheckCircle2, ChevronLeft, ChevronRight, Clock3, Gift, History, Loader2, Plus, Save, Tag, Trash2, X } from 'lucide-react';
...
type PaymentTiming = 'today' | 'past' | 'later';
...
function usesMlQuantity(service: Service) {
  const normalized = service.name.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
  return normalized.includes('preench') || normalized.includes('bioestimul');
}

function serviceQuantity(service: Service, quantities: Record<string, number>) {
  if (!usesMlQuantity(service)) return 1;
  return Math.max(1, Math.min(99, Math.trunc(Number(quantities[service.id] ?? 1) || 1)));
}
...
function newPayment(amount: number, timing: PaymentTiming, performedDate = TODAY): PaymentEntryUI {
  return {
    ...
    installments: 1,
    absorveTaxa: true,
    scheduledDate: timing === 'today' ? TODAY : timing === 'past' ? performedDate : '',
  };
}
...
function ServiceAdjustments({ services, coverage, prices, quantities, courtesy, setPrice, setQuantity, setCourtesy }: { services: Service[]; coverage: Record<string, string | undefined>; prices: Record<string, number>; quantities: Record<string, number>; courtesy: Record<string, boolean>; setPrice: (id: string, value: number) => void; setQuantity: (service: Service, value: number) => void; setCourtesy: (service: Service, value: boolean) => void }) {
  const extras = services.filter(service => !coverage[service.id]);
  if (!extras.length) return null;
  return <section style={{ marginTop: 18 }}><div style={{ marginBottom: 10 }}><strong style={{ display: 'block' }}>Valor realizado</strong><span className="page-sub">O valor do catálogo vem preenchido. Para preenchimento e bioestimulador, ajuste os mL vendidos; o total é recalculado automaticamente.</span></div><div style={{ display: 'grid', gap: 9 }}>{extras.map(service => {
    const qty = serviceQuantity(service, quantities);
    const catalogTotal = Number(service.price) * qty;
    const isGift = Boolean(courtesy[service.id]);
    const finalValue = isGift ? 0 : Number(prices[service.id] ?? catalogTotal);
    const discount = Math.max(0, catalogTotal - finalValue);
    const pct = catalogTotal > 0 ? discount / catalogTotal * 100 : 0;
    return <div key={service.id} style={{ padding: 13, border: `1px solid ${isGift ? '#fbcfe8' : 'var(--border)'}`, borderRadius: 12, background: isGift ? '#fdf2f8' : 'var(--bg-2)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}>
        <div><strong>{service.name}</strong><small className="page-sub" style={{ display: 'block' }}>{usesMlQuantity(service) ? `Catálogo: ${qty} ml × ${money(service.price)} = ${money(catalogTotal)}` : `Catálogo: ${money(catalogTotal)}`}</small></div>
        {isGift && <span className="badge" style={{ background: '#fce7f3', color: '#9d174d', fontWeight: 800 }}><Gift size={12}/> BRINDE</span>}
      </div>
      {usesMlQuantity(service) && <div style={{ marginTop: 10 }}>
        <label className="field-label">Quantidade vendida / aplicada</label>
        <div role="group" aria-label={`Quantidade em ml de ${service.name}`} style={{ width: 'fit-content', minHeight: 46, display: 'grid', gridTemplateColumns: '46px minmax(92px,auto) 46px', alignItems: 'stretch', border: '1px solid var(--border)', borderRadius: 11, overflow: 'hidden', background: 'var(--bg)' }}>
          <button type="button" aria-label={`Diminuir quantidade de ${service.name}`} disabled={qty <= 1} onClick={() => setQuantity(service, qty - 1)} style={{ border: 0, borderRight: '1px solid var(--border)', background: 'transparent', color: 'var(--text)', fontSize: 22, cursor: qty <= 1 ? 'not-allowed' : 'pointer', opacity: qty <= 1 ? .4 : 1 }}>−</button>
          <strong style={{ display: 'grid', placeItems: 'center', padding: '0 14px', fontSize: 16 }}>{qty} ml</strong>
          <button type="button" aria-label={`Aumentar quantidade de ${service.name}`} onClick={() => setQuantity(service, qty + 1)} style={{ border: 0, borderLeft: '1px solid var(--border)', background: 'transparent', color: 'var(--text)', fontSize: 21, cursor: 'pointer' }}>+</button>
        </div>
      </div>}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 10, alignItems: 'end', marginTop: 10 }}>
        <div><label className="field-label">Valor cobrado</label><input className="field-input" type="number" inputMode="decimal" min="0" step="0.01" disabled={isGift} value={finalValue} aria-label={`Valor cobrado: ${service.name}`} onFocus={event => event.currentTarget.select()} onChange={event => setPrice(service.id, Math.max(0, Number(event.target.value) || 0))}/>{!isGift && discount > .009 && <small style={{ color: '#166534', fontWeight: 700 }}><Tag size={11}/> desconto {money(discount)} · {pct.toFixed(0)}%</small>}</div>
        <button type="button" className={`btn btn--md ${isGift ? 'btn--primary' : 'btn--ghost'}`} style={{ minHeight: 44 }} onClick={() => setCourtesy(service, !isGift)}><Gift size={16}/> {isGift ? 'Remover brinde' : 'Marcar como brinde'}</button>
      </div>
    </div>;
  })}</div></section>;
}

function ServiceCostAdjustments({ services, costs, quantities, setCost }: { services: Service[]; costs: Record<string, number>; quantities: Record<string, number>; setCost: (id: string, value: number) => void }) {
  if (!services.length) return null;
  const total = services.reduce((sum, service) => sum + Number(costs[service.id] ?? service.cost_per_unit ?? 0) * serviceQuantity(service, quantities), 0);
  return <section style={{ marginTop: 18 }}><div style={{ marginBottom: 10 }}><strong style={{ display: 'block' }}>Custo real deste atendimento</strong><span className="page-sub">O custo padrão do catálogo é por unidade. Quando houver mais de 1 ml, o total do custo acompanha a quantidade automaticamente.</span></div><div style={{ display: 'grid', gap: 9 }}>{services.map(service => { const qty = serviceQuantity(service, quantities); const unitCost = Number(costs[service.id] ?? service.cost_per_unit ?? 0); return <div key={service.id} style={{ padding: 13, border: '1px solid var(--border)', borderRadius: 12, background: 'var(--bg-2)', display: 'grid', gridTemplateColumns: '1fr minmax(160px,220px)', gap: 12, alignItems: 'center' }}><div><strong>{service.name}</strong><small className="page-sub" style={{ display: 'block' }}>Custo padrão: {money(Number(service.cost_per_unit || 0))}{usesMlQuantity(service) ? ' / ml' : ''}{qty > 1 ? ` · total atual ${money(unitCost * qty)}` : ''}</small></div><div><label className="field-label">{usesMlQuantity(service) ? 'Custo por ml neste atendimento' : 'Custo neste atendimento'}</label><BrlCurrencyInput value={unitCost} onChange={value => setCost(service.id, value)} label={`Custo neste atendimento: ${service.name}`} /></div></div>; })}</div><div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', gap: 12 }}><span>Custo de procedimentos informado</span><strong>{money(total)}</strong></div></section>;
}
...
function PaymentCard({ entry, timing, rates, canRemove, onChange, onRemove }: { entry: PaymentEntryUI; timing: PaymentTiming; rates: MaquininhaRates; canRemove: boolean; onChange: (entry: PaymentEntryUI) => void; onRemove: () => void }) {
  ...
  return <div style={{ padding: 14, border: '1px solid var(--border)', borderRadius: 12, background: 'var(--bg-2)', display: 'grid', gap: 11 }}>
    ...
    <div><label className="field-label">Valor nesta forma de pagamento</label><input className="field-input" type="number" inputMode="decimal" min="0" step="0.01" value={entry.baseValue || ''} onChange={event => onChange({ ...entry, baseValue: Number(event.target.value) || 0 })}/><small className="page-sub">Este campo só divide o total entre as formas de pagamento; não altera o desconto do procedimento.</small></div>
    {card && <div><label className="field-label">Bandeira</label><div style={{ display: 'flex', gap: 7 }}>{(['master_visa', 'elo'] as CardBrand[]).map(brand => <button key={brand} type="button" className={`btn btn--sm ${entry.cardBrand === brand ? 'btn--primary' : 'btn--ghost'}`} onClick={() => onChange({ ...entry, cardBrand: brand })}>{brand === 'master_visa' ? 'Master / Visa' : 'Elo'}</button>)}</div></div>}
    {entry.method === 'cartao_credito' && <div><label className="field-label">Parcelas</label><div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>{Array.from({ length: 18 }, (_, index) => index + 1).map(value => <button key={value} type="button" className={`btn btn--sm ${entry.installments === value ? 'btn--primary' : 'btn--ghost'}`} style={{ minWidth: 42 }} onClick={() => onChange({ ...entry, installments: value })}>{value}x</button>)}</div></div>}
    {amounts.feePct > 0 && <div><label className="field-label">Taxa</label><div style={{ display: 'flex', gap: 7 }}><button type="button" className={`btn btn--sm ${entry.absorveTaxa ? 'btn--primary' : 'btn--ghost'}`} onClick={() => onChange({ ...entry, absorveTaxa: true })}>Clínica absorve</button><button type="button" className={`btn btn--sm ${!entry.absorveTaxa ? 'btn--primary' : 'btn--ghost'}`} onClick={() => onChange({ ...entry, absorveTaxa: false })}>Repassar</button></div><small className="page-sub">Cliente paga {money(amounts.clientPays)} · taxa {money(amounts.feeValue)} · líquido {money(amounts.netAmount)}</small></div>}
    <div>
...
function FinancialStep({ amount, entries, setEntries, rates, timing, setTiming, performedDate, onEditAmount }: { amount: number; entries: PaymentEntryUI[]; setEntries: (entries: PaymentEntryUI[]) => void; rates: MaquininhaRates; timing: PaymentTiming; setTiming: (value: PaymentTiming) => void; performedDate: string; onEditAmount: () => void }) {
  const allocated = entries.reduce((sum, entry) => sum + entry.baseValue, 0);
  const remaining = +(amount - allocated).toFixed(2);
  const choose = (next: PaymentTiming) => {
    setTiming(next);
    const defaultDate = next === 'today' ? TODAY : next === 'past' ? performedDate : '';
    setEntries(entries.length ? entries.map(entry => ({ ...entry, scheduledDate: defaultDate })) : [newPayment(amount, next, performedDate)]);
  };
  return <div>
    <h2 style={{ fontSize: '1.15rem', marginBottom: 5 }}>Situação financeira</h2>
    <p className="page-sub" style={{ marginBottom: 14 }}>Atendimento e recebimento são eventos diferentes. Registre o que realmente aconteceu.</p>
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 9, marginBottom: 16 }}>
      <button type="button" aria-pressed={timing === 'today'} onClick={() => choose('today')} style={{ minHeight: 92, padding: 13, borderRadius: 13, border: `2px solid ${timing === 'today' ? 'var(--primary)' : 'var(--border)'}`, background: timing === 'today' ? 'var(--bg-2)' : 'var(--bg)', color: 'inherit', textAlign: 'left' }}><CheckCircle2 size={20} style={{ color: 'var(--primary)' }}/><strong style={{ display: 'block' }}>Pago hoje</strong><small className="page-sub">Registra o recebimento de hoje.</small></button>
      <button type="button" aria-pressed={timing === 'past'} onClick={() => choose('past')} style={{ minHeight: 92, padding: 13, borderRadius: 13, border: `2px solid ${timing === 'past' ? '#7c3aed' : 'var(--border)'}`, background: timing === 'past' ? '#f5f3ff' : 'var(--bg)', color: 'inherit', textAlign: 'left' }}><History size={20} style={{ color: '#7c3aed' }}/><strong style={{ display: 'block' }}>Pago anteriormente</strong><small className="page-sub">Informe o dia real de cada pagamento.</small></button>
    ...
    <div style={{ padding: 12, marginBottom: 8, borderRadius: 10, background: timing === 'later' ? '#fffbeb' : timing === 'past' ? '#f5f3ff' : '#fdf2f8', display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center' }}><span>{timing === 'later' ? 'Total a receber após descontos' : timing === 'past' ? 'Total já recebido após descontos' : 'Total do atendimento após descontos'}</span><strong>{money(amount)}</strong></div>
...
export function TreatmentRegistrarPageV2() {
  ...
  const amountDue = useMemo(() => Math.max(0, +services.reduce((sum, service) => {
    if (coverage[service.id] || courtesy[service.id]) return sum;
    const catalogTotal = Number(service.price) * serviceQuantity(service, quantities);
    return sum + Number(prices[service.id] ?? catalogTotal);
  }, 0).toFixed(2)), [coverage, courtesy, prices, quantities, services]);
  ...
  useEffect(() => {
    ...
    setCosts(current => Object.fromEntries(services.map(service => [service.id, current[service.id] ?? Number(service.cost_per_unit || 0)])));
  }, [services]);
  ...
  useEffect(() => {
    ...
    void draft.load().then(row => {
      ...
      if (payload.quantityByService) setQuantities(payload.quantityByService);
      if (payload.costByService) setCosts(payload.costByService);
  ...
  useEffect(() => {
    if (hasPayment && step === 3 && !payments.length) setPayments([newPayment(amountDue, timing, performedDate)]);
    if (!hasPayment && payments.length) setPayments([]);
  }, [amountDue, hasPayment, payments.length, performedDate, step, timing]);
  ...
  const saveDraftNow = async () => {
    if (!patient) return;
    const payload: AttendanceDraftPayload = { performedDate, notes, serviceIds: services.map(service => service.id), coverageByService: coverage, finalPriceByService: prices, quantityByService: quantities, costByService: costs, courtesyByService: courtesy, materials, paymentTiming: timing, payments };
  ...
  const confirmAttendance = async () => {
    ...
    try {
      ...
      const procedure = await create({
        ...
        total_cost: services.reduce((sum, service) => sum + Number(costs[service.id] ?? service.cost_per_unit ?? 0) * serviceQuantity(service, quantities), 0),
        payment_method: paymentMethod,
        card_fee_pct: null,
        card_fee_value: computed.reduce((sum, row) => sum + row.feeValue, 0) || null,
        net_value: timing === 'later' ? 0 : computed.reduce((sum, row) => sum + row.netAmount, 0),
        ...
        item_values: services.map(service => {
          const qty = coverage[service.id] ? 1 : serviceQuantity(service, quantities);
          const catalogTotal = Number(service.price) * qty;
          return { service_id: service.id, qty, final_price: coverage[service.id] ? service.price : courtesy[service.id] ? 0 : Number(prices[service.id] ?? catalogTotal) };
        }),
        item_costs: services.map(service => ({ service_id: service.id, cost: Number(costs[service.id] ?? service.cost_per_unit ?? 0) })),
      });
