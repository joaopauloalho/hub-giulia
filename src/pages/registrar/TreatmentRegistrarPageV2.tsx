...
function PaymentCard({ entry, timing, rates, canRemove, onChange, onRemove }: { entry: PaymentEntryUI; timing: PaymentTiming; rates: MaquininhaRates; canRemove: boolean; onChange: (entry: PaymentEntryUI) => void; onRemove: () => void }) {
  ...
  return <div style={{ padding: 14, border: '1px solid var(--border)', borderRadius: 12, background: 'var(--bg-2)', display: 'grid', gap: 11 }}><div style={{ display: 'flex', justifyContent: 'space-between' }}><strong>{timing === 'later' ? 'Recebimento combinado' : 'Pagamento'}</strong>{canRemove && <button type="button" className="icon-btn" onClick={onRemove} aria-label="Remover forma"><Trash2 size={15}/></button>}</div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,minmax(0,1fr))', gap: 6 }}>{(Object.entries(METHOD_LABELS) as [SimplePaymentMethod, string][]).map(([method, label]) => <button key={method} type="button" className={`btn btn--sm ${entry.method === method ? 'btn--primary' : 'btn--ghost'}`} style={{ minHeight: 42 }} onClick={() => onChange({ ...entry, method, installments: 1 })}>{label}</button>)}</div><div><label className="field-label">Valor</label><input className="field-input" type="number" inputMode="decimal" min="0" step="0.01" value={entry.baseValue || ''} onChange={event => onChange({ ...entry, baseValue: Number(event.target.value) || 0 })}/></div>{card && <div><label className="field-label">Bandeira</label><div style={{ display: 'flex', gap: 7 }}>{(['master_visa', 'elo'] as CardBrand[]).map(brand => <button key={brand} type="button" className={`btn btn--sm ${entry.cardBrand === brand ? 'btn--primary' : 'btn--ghost'}`} onClick={() => onChange({ ...entry, cardBrand: brand })}>{brand === 'master_visa' ? 'Master / Visa' : 'Elo'}</button>)}</div></div>}{entry.method === 'cartao_credito' && <div><label className="field-label">Parcelas</label><div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>{Array.from({ length: 18 }, (_, index) => index + 1).map(value => <button key={value} type="button" className={`btn btn--sm ${entry.installments === value ? 'btn--primary' : 'btn--ghost'}`} style={{ minWidth: 42 }} onClick={() => onChange({ ...entry, installments: value })}>{value}x</button>)}</div></div>}{amounts.feePct > 0 && <div><label className="field-label">Taxa</label><div style={{ display: 'flex', gap: 7 }}><button type="button" className={`btn btn--sm ${entry.absorveTaxa ? 'btn--primary' : 'btn--ghost'}`} onClick={() => onChange({ ...entry, absorveTaxa: true })}>Clínica absorve</button><button type="button" className={`btn btn--sm ${!entry.absorveTaxa ? 'btn--primary' : 'btn--ghost'}`} onClick={() => onChange({ ...entry, absorveTaxa: false })}>Repassar</button></div><small className="page-sub">Cliente paga {money(amounts.clientPays)} · taxa {money(amounts.feeValue)} · líquido {money(amounts.netAmount)}</small></div>}{timing === 'later' && <div><label className="field-label">Previsão de recebimento</label><input className="field-input" type="date" min={TOMORROW} value={entry.scheduledDate} onChange={event => onChange({ ...entry, scheduledDate: event.target.value })}/><small className="page-sub">Ficará em A receber até o pagamento ser confirmado.</small></div>}</div>;
}

function FinancialStep({ amount, entries, setEntries, rates, timing, setTiming }: { amount: number; entries: PaymentEntryUI[]; setEntries: (entries: PaymentEntryUI[]) => void; rates: MaquininhaRates; timing: PaymentTiming; setTiming: (value: PaymentTiming) => void }) {
...
export function TreatmentRegistrarPageV2() {
  ...
  const toggleExtra = (service: Service) => { if (!coverage[service.id]) setServices(current => current.some(item => item.id === service.id) ? current.filter(item => item.id !== service.id) : [...current, service]); };
