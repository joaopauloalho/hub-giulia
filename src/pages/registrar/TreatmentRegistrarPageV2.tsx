    ...
    return <div key={service.id} style={{ padding: 13, border: `1px solid ${isGift ? '#fbcfe8' : 'var(--border)'}`, borderRadius: 12, background: isGift ? '#fdf2f8' : 'var(--bg-2)' }}>
      ...
      {usesMlQuantity(service) && <div style={{ marginTop: 10 }}>
        ...
        <div role="group" aria-label={`Quantidade em ml de ${service.name}`} style={{ width: 'fit-content', minHeight: 46, display: 'grid', gridTemplateColumns: '46px minmax(92px,auto) 46px', alignItems: 'stretch', border: '1px solid var(--border)', borderRadius: 11, overflow: 'hidden', background: 'var(--bg)' }}>
          <button type="button" aria-label={`Diminuir quantidade de ${service.name}`} disabled={qty <= 1 || isGift} onClick={() => setQuantity(service, qty - 1)} style={{ border: 0, borderRight: '1px solid var(--border)', background: 'transparent', color: 'var(--text)', fontSize: 22, cursor: qty <= 1 || isGift ? 'not-allowed' : 'pointer', opacity: qty <= 1 || isGift ? .4 : 1 }}>−</button>
          <strong style={{ display: 'grid', placeItems: 'center', padding: '0 14px', fontSize: 16 }}>{qty} ml</strong>
          <button type="button" aria-label={`Aumentar quantidade de ${service.name}`} disabled={isGift} onClick={() => setQuantity(service, qty + 1)} style={{ border: 0, borderLeft: '1px solid var(--border)', background: 'transparent', color: 'var(--text)', fontSize: 21, cursor: isGift ? 'not-allowed' : 'pointer', opacity: isGift ? .4 : 1 }}>+</button>
        </div>
      </div>}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 10, alignItems: 'end', marginTop: 10 }}>
        <div><label className="field-label">Valor cobrado</label><BrlCurrencyInput value={finalValue} onChange={value => setPrice(service.id, value)} label={`Valor cobrado: ${service.name}`} />{!isGift && discount > .009 && <small style={{ color: '#166534', fontWeight: 700 }}><Tag size={11}/> desconto {money(discount)} · {pct.toFixed(0)}%</small>}</div>
