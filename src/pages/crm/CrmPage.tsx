...
import { AlertTriangle, Archive, CalendarPlus, Check, ChevronRight, Clock, MessageCircle, Phone, Plus, Search, StickyNote, UserPlus, X } from 'lucide-react';
import { useCrm, type CrmActivity, type CrmContactCandidate, type CrmFollowup, type CrmInterest, type CrmPatientCandidate, type CrmPipelineCard } from '../../hooks/useCrm';
...
type LossRequest = { card: CrmPipelineCard; target: CrmStage };
...
function Card({ card, onOpen, onMove }: { card: CrmPipelineCard; onOpen: () => void; onMove: (stage: CrmStage) => void }) {
  const bucket = followupBucket(card.next_followup_on);
  const value = formatCrmValue(card.estimated_value);
  return <div className={`crm-card${bucket === 'overdue' ? ' crm-card--overdue' : ''}`} draggable={CRM_OPEN_STAGES.includes(card.stage)} onDragStart={event => event.dataTransfer.setData('text/crm-deal', card.deal_id)}>
    <button type="button" onClick={onOpen} style={{ width: '100%', border: 0, padding: 0, background: 'transparent', textAlign: 'left', cursor: 'pointer' }}>
      ...
      {card.interests.length > 0 && <div className="crm-card-tags">{card.interests.slice(0, 3).map(item => <span className="crm-tag" key={item.id ?? item.label}>{item.label}</span>)}{card.interests.length > 3 && <span className="crm-tag">+{card.interests.length - 3}</span>}</div>}
      <div className="crm-card-meta">
        <span>{CRM_SOURCE_LABEL[card.source]}</span>
        {value && <strong>{value}</strong>}
        {card.patient_id && <span>Paciente vinculada</span>}
      </div>
      {card.next_followup_on && <div className={`crm-followup${bucket ? ` crm-followup--${bucket}` : ''}`}><Clock size={12} style={{ verticalAlign: -2, marginRight: 4 }} />{bucket === 'overdue' ? 'Atrasado · ' : bucket === 'today' ? 'Hoje · ' : 'Próximo · '}{card.next_followup_on.split('-').reverse().join('/')}</div>}
    </button>
...
function LossModal({ request, onClose, onConfirm }: { request: LossRequest; onClose: () => void; onConfirm: (reason: CrmLossReason, detail: string) => Promise<void> }) {
  ...
  return <div className="crm-modal-overlay" onClick={onClose}>
    <form className="crm-modal" onSubmit={submit} onClick={event => event.stopPropagation()}>
...
function NewLeadModal({ patient, onClose, crm }: { patient: PatientSeed | null; onClose: () => void; crm: ReturnType<typeof useCrm> }) {
  ...
  const addInterest = () => {
    ...
    if (!label) return;
    if (form.interests.some(item => item.service_id === (service?.id ?? null) && item.label.toLocaleLowerCase('pt-BR') === label.toLocaleLowerCase('pt-BR'))) return;
    setForm(current => ({
      ...current,
  ...
  const createForExisting = async (contact: CrmContactCandidate) => {
    ...
    try {
      ...
      console.error('[crm:create-existing]', err);
  ...
  return <div className="crm-modal-overlay" onClick={onClose}>
    <form className="crm-modal" onSubmit={event => { event.preventDefault(); void submit(false); }} onClick={event => event.stopPropagation()}>
      <div className="crm-modal-head"><div style={{ flex: 1 }}><strong>{patient ? 'Nova oportunidade' : 'Novo lead'}</strong><div className="page-sub">{patient ? `Paciente: ${patient.name}` : 'Contato + oportunidade em uma operação atômica'}</div></div><button type="button" className="icon-btn" onClick={onClose}><X size={18} /></button></div>
      <div className="crm-modal-body">
...
function DetailDrawer({ card, crm, onClose, onStage }: { card: CrmPipelineCard; crm: ReturnType<typeof useCrm>; onClose: () => void; onStage: (stage: CrmStage) => void }) {
  ...
  const reload = async () => {
    setLoading(true);
    try { setDetail(await crm.loadDetail(card.deal_id)); } catch (err) { console.error('[crm:detail]', err); toast.error('Não foi possível carregar os detalhes.'); } finally { setLoading(false); }
  };
  ...
  const registerContact = async () => {
    setBusy(true);
    try { await crm.recordContact(card, contactChannel, contactNote); setContactOpen(false); setContactNote(''); await reload(); toast.success('Contato registrado.'); } catch { toast.error('Não foi possível registrar o contato.'); } finally { setBusy(false); }
  };

  const createFollowup = async () => {
    setBusy(true);
    try { await crm.addFollowup(card.deal_id, followupDate, followupChannel, followupNote); setFollowupOpen(false); setFollowupNote(''); await reload(); toast.success('Follow-up criado.'); } catch { toast.error('Não foi possível criar o follow-up.'); } finally { setBusy(false); }
  };

  const saveNote = async () => {
    if (!note.trim()) return;
    setBusy(true);
    try { await crm.addNote(card, note); setNote(''); setNoteOpen(false); await reload(); toast.success('Nota registrada.'); } catch { toast.error('Não foi possível salvar a nota.'); } finally { setBusy(false); }
  };
  ...
  const startConversion = async () => {
    ...
    try {
      ...
      onClose();
    } catch (err) { console.error('[crm:convert]', err); toast.error('Não foi possível converter o contato.'); } finally { setBusy(false); }
  };

  const linkPatient = async (patientId: string) => {
    setBusy(true);
    try { await crm.convertContact(card.contact_id, patientId); toast.success('Paciente vinculada ao contato.'); setCandidates(null); onClose(); } catch { toast.error('Não foi possível vincular a paciente.'); } finally { setBusy(false); }
  };
  ...
  const createSeparatePatient = async () => {
    ...
    if (!ok) return;
    setBusy(true);
    try { await crm.convertContact(card.contact_id); toast.success('Nova paciente criada e vinculada.'); onClose(); } catch { toast.error('Não foi possível criar a paciente.'); } finally { setBusy(false); }
  };
  ...
  const activityLabel = (activity: CrmActivity) => {
    if (activity.activity_type === 'stage_changed') return `${activity.from_stage ? CRM_STAGE_LABEL[activity.from_stage] : 'Criada'} → ${activity.to_stage ? CRM_STAGE_LABEL[activity.to_stage] : ''}`;
  ...
  return <div className="drawer-overlay" onClick={onClose}>
    <aside className="drawer crm-drawer" role="dialog" aria-modal="true" onClick={event => event.stopPropagation()}>
      ...
      <div className="crm-detail-hero">
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', fontSize: 12, color: 'var(--text-2)' }}>{card.phone && <span><Phone size={12} style={{ verticalAlign: -2 }} /> {card.phone}</span>}{card.email && <span>{card.email}</span>}<span>{CRM_SOURCE_LABEL[card.source]}{card.source_detail ? ` · ${card.source_detail}` : ''}</span>}{formatCrmValue(card.estimated_value) && <strong>{formatCrmValue(card.estimated_value)}</strong>}</div>
...
export function CrmPage() {
  ...
  const boardStages = stage === 'all' ? CRM_OPEN_STAGES : [stage];
  ...
  const requestMove = async (card: CrmPipelineCard, target: CrmStage) => {
    ...
    if (target === 'lost') { setLossRequest({ card, target }); return; }
    try { await crm.moveStage(card.deal_id, target); if (selected?.deal_id === card.deal_id) setSelected({ ...card, stage: target }); toast.success(`Movido para ${CRM_STAGE_LABEL[target]}.`); } catch { toast.error('Não foi possível mover a oportunidade.'); }
  };

  const dropInto = async (event: React.DragEvent<HTMLDivElement>, target: CrmStage) => {
    event.preventDefault();
  ...
  return <div className="page crm-page">
    ...
    {crm.loading ? <div className="card" style={{ padding: 16 }}>Carregando CRM…</div> : crm.error ? <div className="empty-state"><p>{crm.error}</p><button className="btn btn--secondary btn--sm" onClick={() => void crm.refresh()}>Tentar novamente</button></div> : <>
      <div className="crm-board">{boardStages.map(columnStage => { const items = crm.cards.filter(card => card.stage === columnStage); return <div className="crm-column" key={columnStage} onDragOver={event => event.preventDefault()} onDrop={event => void dropInto(event, columnStage)}><div className="crm-column-head"><span className="crm-column-title">{CRM_STAGE_LABEL[columnStage]}</span><span className="crm-column-count">{items.length}</span></div>{items.length === 0 ? <div className="crm-empty-column">Nenhuma oportunidade</div> : items.map(card => <Card key={card.deal_id} card={card} onOpen={() => setSelected(card)} onMove={target => void requestMove(card, target)} />)}</div>; })}</div>
      <div className="crm-mobile-list">{mobileCards.length === 0 ? <div className="empty-state">Nenhuma oportunidade neste filtro.</div> : mobileCards.map(card => <Card key={card.deal_id} card={card} onOpen={() => setSelected(card)} onMove={target => void requestMove(card, target)} />)}</div>
    </>}
