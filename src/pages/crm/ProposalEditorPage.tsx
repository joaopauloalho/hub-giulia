...
export function ProposalEditorPage() {
  ...
  const handleSave = async () => { if (!detail || !editor) return; setBusyAction('save'); try { if (!editor.items.length) throw new Error('Adicione pelo menos um procedimento.'); await flush(); if (advanceCrm) { await advanceProposalCrm(detail.proposal.id); toast.success('Proposta salva e movida para Proposta enviada no CRM.'); } else toast.success('Proposta salva.'); } catch (error) { toast.error(proposalErrorMessage(error)); } finally { setBusyAction(null); } };
