...
export function useCrm(filters: CrmFilters = {}) {
  ...
  const moveStage=async(dealId:string,stage:CrmStage,loss?:{reason:CrmLossReason;detail?:string|null})=>{const payload:{stage:CrmStage;recontact_on:null;recontact_note:null;lost_reason?:CrmLossReason;lost_reason_detail?:string|null}={stage,recontact_on:null,recontact_note:null};if(stage==='lost'&&loss){payload.lost_reason=loss.reason;payload.lost_reason_detail=loss.detail?.trim()||null;}const{error}=await supabase.from('deals').update(payload).eq('id',dealId);if(error)throw error;await load();};
