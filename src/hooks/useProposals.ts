...
import type {
  ...
  ProposalSummary,
  TreatmentProposal,
  TreatmentProposalItem,
  TreatmentProposalVersion,
} from '../lib/proposals';
...
export type ProposalDetail = {
  ...
  versions: TreatmentProposalVersion[];
  version: TreatmentProposalVersion;
  items: TreatmentProposalItem[];
};

export async function loadProposalDealContext(dealId: string): Promise<ProposalDealContext> {
  const { data, error } = await supabase.from('crm_pipeline_v').select('*').eq('deal_id', dealId).single();
  if (error) throw error;
  return data as ProposalDealContext;
}

export async function createProposal(dealId: string, title: string) {
  const { data, error } = await supabase.rpc('create_treatment_proposal_v1', {
    p_deal_id: dealId,
    p_title: title,
    p_idempotency_key: crypto.randomUUID(),
  });
  if (error) throw error;
  const row = (data as Array<{ proposal_id: string; version_id: string; version_number: number; draft_revision: number }> | null)?.[0];
  if (!row) throw new Error('PROPOSAL_CREATE_NOT_CONFIRMED');
  return row;
}

export async function loadDealProposals(dealId: string): Promise<ProposalSummary[]> {
  const { data, error } = await supabase
    .from('treatment_proposal_summary_v')
    .select('*')
    .eq('deal_id', dealId)
...
export async function loadProposal(proposalId: string, preferredVersionId?: string | null): Promise<ProposalDetail> {
  ...
  const rows = (versions ?? []) as TreatmentProposalVersion[];
  const version = rows.find(item => item.id === preferredVersionId) ?? rows[0];
  ...
  return { proposal: proposal as TreatmentProposal, versions: rows, version, items: (items ?? []) as TreatmentProposalItem[] };
}
...
export async function saveProposalDraft(input: {
  ...
  const { data, error } = await supabase.rpc('save_treatment_proposal_draft_v1', {
    ...
    p_items: input.items.map((item, index) => ({
      ...
      interval_note: item.interval_note || null,
      quantity: item.quantity,
      unit_label: item.unit_label,
      list_unit_price_snapshot: item.list_unit_price_snapshot || '0',
      offered_unit_price: item.offered_unit_price || '0',
  ...
  if (!row) throw new Error('PROPOSAL_SAVE_NOT_CONFIRMED');
  return row;
}

export async function issueProposal(versionId: string, expectedRevision: number, idempotencyKey: string) {
  const { data, error } = await supabase.rpc('issue_treatment_proposal_v1', {
    p_version_id: versionId,
    p_expected_revision: expectedRevision,
    p_idempotency_key: idempotencyKey,
  });
  if (error) throw error;
  return (data as unknown[] | null)?.[0];
}

export async function createProposalRevision(sourceVersionId: string, idempotencyKey = crypto.randomUUID()) {
  const { data, error } = await supabase.rpc('create_treatment_proposal_revision_v1', {
    p_source_version_id: sourceVersionId,
    p_idempotency_key: idempotencyKey,
  });
  if (error) throw error;
  const row = (data as Array<{ proposal_id: string; version_id: string; version_number: number; draft_revision: number }> | null)?.[0];
  if (!row) throw new Error('PROPOSAL_REVISION_NOT_CONFIRMED');
  return row;
}

export async function markProposalSent(versionId: string, idempotencyKey = crypto.randomUUID()) {
  const { data, error } = await supabase.rpc('mark_treatment_proposal_sent_v1', { p_version_id: versionId, p_idempotency_key: idempotencyKey });
  if (error) throw error;
  return (data as unknown[] | null)?.[0];
}

export async function acceptProposal(versionId: string, markDealWon: boolean, idempotencyKey = crypto.randomUUID()) {
  const { data, error } = await supabase.rpc('accept_treatment_proposal_v1', {
    p_version_id: versionId,
    p_mark_deal_won: markDealWon,
    p_idempotency_key: idempotencyKey,
  });
  if (error) throw error;
  return (data as unknown[] | null)?.[0];
...
export async function declineProposal(versionId: string, reason: string, idempotencyKey = crypto.randomUUID()) {
  const { data, error } = await supabase.rpc('decline_treatment_proposal_v1', { p_version_id: versionId, p_reason: reason || null, p_idempotency_key: idempotencyKey });
  if (error) throw error;
  return (data as unknown[] | null)?.[0];
}

export async function voidProposal(versionId: string, reason: string, idempotencyKey = crypto.randomUUID()) {
  const { data, error } = await supabase.rpc('void_treatment_proposal_v1', { p_version_id: versionId, p_reason: reason, p_idempotency_key: idempotencyKey });
  if (error) throw error;
  return (data as unknown[] | null)?.[0];
}
...
export async function uploadProposalPdf(proposalId: string, versionId: string, blob: Blob) {
  ...
  if (authError || !authData.user) throw new Error('PROPOSAL_SESSION_REQUIRED');
  const path = `${authData.user.id}/${proposalId}/${versionId}/proposal.pdf`;
  let artifact = blob;
  ...
  if (error) {
    ...
    const { data: existing, error: downloadError } = await supabase.storage.from('proposals').download(path);
    if (downloadError || !existing) throw downloadError ?? error;
    artifact = existing;
  }
  const hash = await sha256(artifact);
  const { error: attachError } = await supabase.rpc('attach_treatment_proposal_pdf_v1', {
    p_version_id: versionId,
    p_pdf_path: path,
    p_pdf_sha256: hash,
  });
  if (attachError) throw attachError;
  return { path, hash, blob: artifact };
}

export async function loadProposalPdf(path: string) {
  const { data, error } = await supabase.storage.from('proposals').download(path);
  if (error) throw error;
  return data;
}

export async function proposalSignedUrl(path: string | null | undefined) {
  return createSignedStorageUrl('proposals', path);
}

export async function shareProposalFile(blob: Blob, title: string) {
  const safeName = `${title || 'proposta'}.pdf`.replace(/[^a-zA-Z0-9À-ÿ._ -]+/g, '').replace(/\s+/g, '-');
